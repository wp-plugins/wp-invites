...
class wp_invites
{
    ...
    function init()
    {
        ...
        add_action('admin_menu', array($this, 'invites_add_admin_menu'), 20);
...
//  WPMU starts trying to tell you that you don't have access
    ...
    function invites_ifreal($val)#check if it's a real invite code
    {
        ...
        $res = $wpdb->get_var($wpdb->prepare(
            "SELECT 1 FROM " . $this->options['INVITES_PREFIX'] . "invites WHERE `value`= %s LIMIT 1", $i));
        if (is_null($res)) {
...
';
            ...
            <form method="post" action="">
                <table>
                    ...
                    <tr>
                        ...
                        <td><input name="wp_invites[REMOVE_INTERVAL]"
                    ...
                    <tr>
                        <td><?php _e('Enable debug output', 'wp-invites'); ?></td>
    ...
    function invites_add_signup_fields($errors_mu)
    {
        ...
        <p>
            ...
            <input type="text" name="invite_code" value="<?php echo $_REQUEST['invite_code']; ?>"
    ...
    {
        global $wpdb;
        $sql = 'DELETE FROM ' . $this->options['INVITES_PREFIX'] . 'invites WHERE `value`="' . $this->invites_unbeautify($_SESSION['invite_code']) . '"';
        $res = $wpdb->query($sql);
        if ($res === FALSE)
            echo $wpdb->last_error;
    ...
    {
        $code = $this->invites_beautify(get_usermeta($user->ID, 'invite_code'));
        if (!$code)
    ...
    function bp_output_invites($id)
    {
        ...
        if ($bp->current_component != 'profile')
            return;
        $code = $this->invites_beautify(get_usermeta($bp->displayed_user->id, 'invite_code'));
        if (!$code)
            $code = __('No code assigned', 'wp-invites');
